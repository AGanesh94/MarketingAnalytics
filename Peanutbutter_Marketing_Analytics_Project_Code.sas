...
LIBNAME pea "C:\Users\axg180019\Desktop\Predictive\Project";
...
/* ------ Checking Market Share and Overall Trend ----------- */

DATA pea.Sales_Trend_Calc_1;
SET pea.sales_1;

IF FIND(L5,'JIF','i') GE 1 THEN BRAND = 1;

IF FIND(L5,'SKIPPY','i') GE 1 THEN BRAND = 2;

IF FIND(L5,'PETER','i') GE 1 THEN BRAND = 3;

IF FIND(L5,'PRIVATE','i') GE 1 THEN BRAND = 4;

RUN;


DATA pea.Sales_Trend_Calc_2;
SET pea.Sales_Trend_Calc_1;

IF BRAND = '.' THEN BRAND = 5;

RUN;

/* ------ Considering the Market share from only those stores that have all 52 weeks of data 
to maintain uniformity ----------- */

PROC SQL;
CREATE TABLE pea.Sales_Trend_Calc_3 AS
SELECT IRI_KEY,COUNT(DISTINCT WEEK) AS DISTINCT_WEEKS FROM pea.Sales_Trend_Calc_2
GROUP BY IRI_KEY
...
PROC SQL;
CREATE TABLE pea.Sales_Trend_Calc_4 AS
SELECT * FROM pea.Sales_Trend_Calc_2 WHERE IRI_KEY IN
(SELECT IRI_KEY FROM pea.Sales_Trend_Calc_3 );
QUIT;

PROC SQL;
CREATE TABLE pea.Sales_Trend_Calc_5 AS
SELECT IRI_KEY,WEEK,BRAND, SUM(DOLLARS) AS Amt  FROM pea.Sales_Trend_Calc_4
GROUP BY IRI_KEY,WEEK,BRAND;
QUIT;


/* Exporting grouped dataset */
proc export data=pea.Sales_Trend_Calc_5
outfile="C:\Users\axg180019\Desktop\Predictive\Project\Sales_Trend_Calc.csv" dbms=CSV; run;




/* --------- Analyzing Skippy Data as our chosen Brand ------------ */
...
PROC SQL;
CREATE TABLE pea.Peanut_Grouped AS SELECT iri_key,week,prod_id,SUM(units) AS tot_units,SUM(tot_vol) 
AS SUM_vol,SUM(dollars) AS tot_dollars,
